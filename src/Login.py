...
def LogInWebsite():
    if AUTO:
        ...
        with sync_playwright() as p:
            ...
            #======================================================================================
            # 如果page.get_by_role("img", name="用户063576163573的头像")是在页面加载完成后才会显示的，那么在这个元素出现之后再保存cookies应该是可以的。
            # 但是，context.storage_state(path='./auth/CrawCookies.json')这行代码可能在print(page.get_by_role("img", name="用户063576163573的头像").get_attribute('src'))这行代码之前就已经执行了，因为print函数可能会花费一些时间来获取元素的属性。
            # 为了确保在保存cookies之前页面已经加载完成，您可以在print函数之后添加一个page.wait_for_timeout(1000)，以确保有足够的时间让页面加载完成。
            # 保存状态文件
            #======================================================================================
            
            context.storage_state(path='../auth/CrawCookies.json')
            page.wait_for_timeout(1000)
            return page
